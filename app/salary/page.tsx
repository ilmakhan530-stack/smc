...
type Advance={personId:string;personType:"labour"|"staff";personName:string;amount:number;date:string;note?:string};
type SalaryDoc={month:string;personId:string;personType:string;balance:number;deduction?:number;paymentStatus?:"Paid"|"Unpaid";paymentNotes?:string};
...
export default function SalaryPage(){
 ...
 async function save(p:Person){
   ...
   try{
     await setDoc(doc(db,"salary",`${month}_${p.type}_${p.id}`),{
       ...
       paymentStatus:x.paymentStatus,paymentNotes:x.paymentNotes,updatedAt:new Date().toISOString()
     })
 ...
 return <AuthGuard requiredPermission="salary"><div style={{display:"flex",minHeight:"100vh"}}><Sidebar/><main style={{flex:1,padding:"30px 34px",minWidth:0}}>
