...
type SalaryDoc={month:string;personId:string;personType:string;balance:number};
...
export default function SalaryPage(){
 ...
 function calc(p:Person){
   ...
   const sundayWork=rows.filter(a=>isSunday(a.date)&&p.type==="labour"&&a.status!=="Absent");
   const present=regular.filter(a=>a.status==="Present").length, absent=regular.filter(a=>a.status==="Absent").length, half=regular.filter(a=>a.status==="Half Day").length;
   const payableDays=present+half*.5, dayRate=salaryDays?Number(p.monthlySalary||0)/salaryDays:0;
   const regularEarned=dayRate*payableDays;
   const sundayPay=sundayWork.reduce((n,a)=>n+dayRate*(a.status==="Half Day"?.5:1)*2,0);
   const overtimeHours=rows.reduce((n,a)=>n+Number(a.overtimeHours||0),0), overtimePay=(dayRate/8)*overtimeHours;
   ...
   const prev=previousMonth(month); const previousSaved=savedSalaries.filter(s=>s.personId===p.id&&s.personType===p.type&&s.month===prev)[0];
   const openingBalance=Number(previousSaved?.balance||0);
   const finalBalance=Math.round(openingBalance+earned-advanceTotal);
   return {rows,present,absent,half,payableDays,earned,sundayPay:Math.round(sundayPay),overtimeHours,overtimePay:Math.round(overtimePay),advanceTotal,openingBalance,finalBalance};
 }
 async function save(p:Person){const s=calc(p),key=`${p.type}_${p.id}`;setSaving(key);setError("");try{await setDoc(doc(db,"salary",`${month}_${p.type}_${p.id}`),{month,personId:p.id,personType:p.type,personName:p.name,monthlySalary:p.monthlySalary,presentDays:s.present,absentDays:s.absent,halfDays:s.half,payableDays:s.payableDays,earnedSalary:s.earned,sundayDoublePay:s.sundayPay,overtimeHours:s.overtimeHours,overtimePay:s.overtimePay,advance:s.advanceTotal,openingBalance:s.openingBalance,balance:s.finalBalance,finalBalance:s.finalBalance,updatedAt:new Date().toISOString()})}catch(e:any){setError(e?.message||"Unable to save salary")}finally{setSaving(null)}}
 ...
 return <AuthGuard allowedRoles={["admin"]}><div style={{display:"flex",minHeight:"100vh"}}><Sidebar/><main style={{flex:1,padding:"30px 34px",minWidth:0}}>
  ...
  <div style={{display:"flex",gap:12,marginBottom:16,alignItems:"center"}}><div style={{position:"relative",width:330,maxWidth:"100%"}}><Search size={17} style={{position:"absolute",left:12,top:12,color:"#7b8ca5"}}/><input className="input" style={{paddingLeft:38}} placeholder="Search name or mobile" value={search} onChange={e=>setSearch(e.target.value)}/></div><div style={{color:"#71809a",fontSize:12}}>Monthly salary ÷ calendar days • Sunday Labour = 2× • OT = hours from In/Out • Advance is separate • Negative balance carries forward.</div></div>
