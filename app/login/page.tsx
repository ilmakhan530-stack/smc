...
export default function Login(){
 ...
 async function submit(e:FormEvent){e.preventDefault();setError("");setLoading(true);try{const cred=await signInWithEmailAndPassword(auth,email.trim(),password);const snap=await getDoc(doc(db,"users",cred.user.uid));if(!snap.exists())throw new Error("User role is not configured.");const data=snap.data() || {};
const role=data?.role;
const permissions=(data?.permissions || {}) as Record<string, boolean>;
const next=role==="admin" ? "/dashboard" :
  permissions.dashboard ? "/dashboard" :
  permissions.attendance ? "/attendance" :
  permissions.bill ? "/bill" :
  permissions.stock ? "/stock" :
  permissions.labour ? "/labour" :
  permissions.staff ? "/staff" :
  permissions.salary ? "/salary" :
  permissions.advance ? "/advance" :
  permissions.thekedar ? "/thekedar" :
  permissions.reports ? "/reports" : "/login";
router.replace(next);}catch(err:any){setError(err?.code||err?.message||"Login failed");}finally{setLoading(false);}}
