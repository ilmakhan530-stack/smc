      ...
      <main style={{flex:1,padding:"28px",minWidth:0}}>
        ...
        <div className="invoice-wrap">
          <div className="invoice">
            <div className="invoice-top"><div><div className="invoice-title">Tax Invoice</div><div className="small">SMC OFFICE MANAGEMENT SYSTEM</div></div><div style={{textAlign:"right"}}><b>Invoice No.: {invoice}</b><br/>Date: {date}<br/><b>(ORIGINAL FOR RECIPIENT)</b></div></div>
          ...
          <div className="invoice duplicate"><div className="copy-label">DUPLICATE FOR TRANSPORTER</div>
            <div className="invoice-top"><div><div className="invoice-title">Tax Invoice</div><div className="small">SMC OFFICE MANAGEMENT SYSTEM</div></div><div style={{textAlign:"right"}}><b>Invoice No.: {invoice}</b><br/>Date: {date}</div></div>
            <div className="invoice-meta"><div><b>Seller (Bill From)</b><div className="party-name">{seller.name||"—"}</div><div>{seller.address||"—"}</div><div>GSTIN/UIN: {seller.gstin||"—"}</div></div><div><b>Buyer (Bill To)</b><div className="party-name">{buyer.name||"—"}</div><div>{buyer.address||"—"}</div><div>GSTIN/UIN: {buyer.gstin||"—"}</div></div><div><div>Delivery Note: {deliveryNote||"—"}</div><div>Buyer's Order No.: {buyersOrder||"—"}</div><div>Dispatched Through: {dispatchedThrough||"—"}</div><div>Destination: {destination||"—"}</div></div></div>
            <table className="invoice-table"><thead><tr><th>S.No.</th><th>Description of Goods</th><th>HSN/SAC</th><th>Quantity</th><th>Rate</th><th>Amount</th></tr></thead><tbody>{items.filter(x=>x.description.trim()).map((x,i)=><tr key={i}><td>{i+1}</td><td><b>{x.description}</b><div className="tax-inline">{taxMode==="igst"?`IGST @ ${rate}%`:taxMode==="cgst_sgst"?`CGST @ ${rate/2}% + SGST @ ${rate/2}%`:""}</div></td><td>{x.hsn}</td><td>{x.quantity} {x.unit}</td><td>{money(x.rate)}</td><td>{money(x.quantity*x.rate)}</td></tr>)}<tr><td colSpan={5} style={{textAlign:"right"}}><b>Grand Total</b></td><td><b>{money(grandTotal)}</b></td></tr></tbody></table>
            <div className="words"><b>Amount Chargeable (in words)</b><br/>{numberToWords(grandTotal)}</div><div className="signature"><b>for {seller.name||"Seller"}</b><div>Authorised Signatory</div></div><div className="generated">This is a Computer Generated Invoice</div>
          </div>
        </div>
      </main>
      <style jsx global>{`
