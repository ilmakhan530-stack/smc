...
export default function SalaryEmployeeDetail({open,onClose,name,type,month,monthlySalary,openingBalance,earnedSalary,sundayPay,overtimePay,advanceTotal,advanceHistory,finalBalance,rows}:{open:boolean;onClose:()=>void;name:string;type:string;month:string;monthlySalary:number;openingBalance:number;earnedSalary:number;sundayPay:number;overtimePay:number;advanceTotal:number;advanceHistory:AdvanceRow[];finalBalance:number;rows:DetailRow[]}){
 if(!open)return null;
 ...
 return <div onMouseDown={e=>e.target===e.currentTarget&&onClose()} style={{position:"fixed",inset:0,zIndex:100,background:"rgba(4,25,55,.48)",display:"grid",placeItems:"center",padding:18}}>
  <div style={{background:"#fff",borderRadius:18,width:1050,maxWidth:"100%",maxHeight:"92vh",overflow:"auto",padding:26}}>
   ...
   <div style={{display:"grid",gridTemplateColumns:"repeat(5,minmax(0,1fr))",gap:10,marginTop:18}}>{[["Opening Balance",openingBalance],["Monthly Earned",earnedSalary],["Sunday Labour Pay",sundayPay],["OT Pay",overtimePay],["Advance",advanceTotal]].map(([l,v])=><div key={String(l)} style={{padding:13,border:"1px solid #e8eef7",borderRadius:12}}><small style={{color:"#71809a"}}>{l}</small><b style={{display:"block",marginTop:5,color:"#17345f"}}>₹ {Number(v).toLocaleString("en-IN")}</b></div>)}</div>
